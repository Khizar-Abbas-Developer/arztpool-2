...
import './ServiceAreaFouthLayout.css'
import { Form } from 'react-bootstrap'
import { useState } from 'react'

const FouthLayout = ({ userId, sittingServicesAreaArray }) => {
  const [currentNumber, setCurrentNumber] = useState(null)
  const URL = process.env.NEXT_PUBLIC_BACK_END_API_URL
  const [listOfFederalStates, setListOfFederalStates] = useState([])
  const [newData, setNewData] = useState(sittingServicesAreaArray)
  ...
  return (
    <>
      <div className="user-table-containerst-2 my-16">
        <div className="text-business-heading">Sitzdienste</div>
        <main className="user-table-containerst" style={{ width: '100%' }}>
          <div className="flex flex-col gap-2">
            {Array.isArray(sittingServicesAreaArray) &&
              sittingServicesAreaArray.map((item, index) => {
                return (
                                  ...
                                  <div className="sub-container">
                                    <div className="entryarea">
                                      <input
                                        ...
                                        name={'abbreviation'}
                                        value={
                                          newData[index].abbreviation || ''
                                        } // Access data dynamically using index
                                        onChange={(e) =>
                                          handleInputChange(e, index)
                                        } // Pass index to handleInputChange
                                        required
